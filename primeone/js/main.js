...
function adaptive_header(w,h) {
		...
		var headerContacts=$('.header-top-lang');
	if(w<767){
	...
	var headerContacts=$('.header-bottom-menu');
	if(w<767){
		if(!headerContacts.hasClass('done')){
			headerContacts.addClass('done').appendTo(headerMenu);
		}
		...
		$.each($('.header-bottom-menu'), function(index,val){
			if($(this).hasClass('header-bottom-menu-list--right')){
				if($(this).hasClass('done')){ 
					$(this).removeClass('done').prependTo($('.header-bottom__coiumn').eq(2));
		} 
			} else{
				if($(this).hasClass('done')){
					$(this).removeClass('done').prependTo($('.header-bottom__coiumn').eq(0));
			}}
